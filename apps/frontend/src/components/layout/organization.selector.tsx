...
import { useOrganizationRequest } from "@growchief/frontend/requests/organization.request.ts";
export const OrganizationSelector: FC<{ asOpenSelect?: boolean }> = ({
  asOpenSelect,
}) => {
  const user = useUser();
  const fetch = useFetch();
  const organizations = useOrganizationRequest();
  const { isLoading, data } = organizations.list();

  const changeOrg = useCallback(
    (org: { companyName: string; id: string }) => async () => {
      await fetch("/organizations/change", {
        method: "POST",
        body: JSON.stringify({
          id: org.id,
        }),
  ...
  return (
    <>
      {!asOpenSelect && <div className="w-[1px] h-[24px] bg-secondary/20" />}
      <div className="hover:text-newTextColor">
        <div className="group text-[12px] relative">
          {asOpenSelect && (
            <div className="bg-btnPrimary !flex !relative py-[12px]">
              Change Organization
            </div>
          )}
          {!asOpenSelect && (
          ...
          {data?.length > 1 && (
            <div
              className={clsx(
                "hidden group-hover:flex absolute top-[100%] end-0 overflow-hidden bg-innerBackground rounded-[8px] cursor-pointer flex-col",
                asOpenSelect ? "!flex !relative mb-[10px]" : "",
              )}
            >
              {data?.map((org: { companyName: string; id: string }) => (
                <div
                  key={org.id}
                  onClick={changeOrg(org)}
                  className="py-[12px] px-[12px] hover:bg-btn-primary hover:text-primary border border-background"
                >
                  {org.companyName}
                </div>
              ))}
